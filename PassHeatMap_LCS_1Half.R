...
#for first half only
lcs.pass1 <- subset(df,Event=='Successful Pass' & Team=='LCS(H)' & Half=='First' )
...
visual1<- ggplot(data = lcs.pass1, aes(x=X, y=Y)) + 
  ...
  #Labels
  ggtitle("Heatmap for LCS 1st-half Passes ========>" ,subtitle = (label=paste("No. of LCS passes=",total.pass)))+
  labs(color= 'Player')+
