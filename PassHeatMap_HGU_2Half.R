...
#for first half only
lcs.pass1 <- subset(df,Event=='Successful Pass' & Team=='HGU(A)' & Half=='Second' )
...
visual1<- ggplot(data = lcs.pass1, aes(x=X, y=Y)) + 
  ...
  #Labels
  ggtitle("Heatmap for HGU 2nd-half Passes =====>" ,subtitle = (label=paste("No. of HGU passes=",total.pass)))+
  labs(color= 'Player')+
