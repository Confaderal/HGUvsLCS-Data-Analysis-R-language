...
cross.map <- subset(df,Event=='Throw' & Half=='Second')
...
final.plot <- ggplot(cross.map,aes(color=Team)) +
  ...
  ggtitle("Throw-ins per half", subtitle = '<====LCS/HGU====> Second Half')+
  theme(plot.title = element_text(face = "bold"))+
  annotate('text',7,4,
           label=paste("No. of LCS throw-ins=",crossLCS),
           parse=FALSE,
           hjust=0.4, size=3.5)+
  annotate('text',7,9,
           label=paste("No. of HGU throw-ins=",crossHGU),
           parse=FALSE,
           hjust=0.4, size=3.5)
